...
class DataBase:
    ...
    def loadAccount(self):
        ...
        if user_data:
            ...
            self.player.keysReward = user_data[0]["info"]["keyReward"]
            self.player.trophiesReward = user_data[0]["info"]["trReward"]
            self.player.tickets = user_data[0]["info"]["tickets"]
            self.player.tokensdoubler = user_data[0]["info"]["tokensdoubler"]
            self.player.availabletokens = user_data[0]["info"]["availableTokens"]
            self.player.starkeysReward = user_data[0]["info"]["skReward"]
            ...
            self.player.ownSkinsArray = user_data[0]["info"]["ownSkinsArray"]
            ...
            "info":
                {
                    ...
                    "ownSkinsArray": self.player.ownSkinsArray,
    ...
    def appendElementToArray(self, value_name, element):
        ...
        data = db.search(query.token == str(self.player.token))
        user_data = data[0]
        user_data["info"][str(value_name)].append(element)
        db.update(user_data, query.token == str(self.player.token))
