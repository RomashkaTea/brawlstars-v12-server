...
class Leave_Message(BSMessageReader):
    ...
    def process(self, crypter):
        ...
        AllianceLeaveOkMessage(self.client, self.player).send(crypter)
        MyAllianceMessage(self.client, self.player, 0).send(crypter)
        for player in self.plrids:
            if player != self.player.low_id:
                AllianceDataMessage(self.client, self.player, 0, self.player.club_low_id).sendWithLowID(player, crypter)
                AllianceChatServerMessage(self.client, self.player, 4).sendWithLowID(player, crypter)
