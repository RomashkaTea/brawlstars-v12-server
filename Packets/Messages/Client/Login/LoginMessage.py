...
class LoginMessage(BSMessageReader):
    ...
    def process(self, crypter):
        if self.major != 12:
            ...
            LoginFailedMessage(self.client, self.player, "").send(crypter)
        ...
        if self.fingerprint_sha != self.player.patch_sha and self.player.patch:
            ...
            try:
                MyAllianceMessage(self.client, self.player, self.player.club_low_id).send(crypter)
                AllianceStreamMessage(self.client, self.player, self.player.club_low_id, 0).send(crypter)
                DataBase.GetmsgCount(self, self.player.club_low_id)
            except:
                MyAllianceMessage(self.client, self.player, 0).send(crypter)
                AllianceStreamMessage(self.client, self.player, 0, 0).send(crypter)
